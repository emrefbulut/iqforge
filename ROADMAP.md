...
- [x] Optional CUDA for **new** measurements only. Published tables and the
      ...
      by default — CUDA is `--device cuda`, never a silent fallback because a
      GPU is present. Record `cpu` / `cuda` in `TrainingResult.environment`
      (or equivalent) so two tables can be told apart. A CUDA run is a new
      labeled measurement (the real-recording leakage curve, when that
      happens), not a replacement of `artifacts/leakage_*.json`. Do not claim
      GPU bit-equality. Do not enlarge *n* after seeing results.
