...
Usage:
    ...
    uv run python scripts/leakage_real.py
"""
...
def run(
    levels: tuple[float | None, ...], split_seeds: list[int], train_seeds: list[int]
) -> list[Run]:
    runs: list[Run] = []
    work = Path(tempfile.mkdtemp(prefix="iqforge-real-"))
    total = len(levels) * len(split_seeds) * len(train_seeds) * 2
    done = 0
    started = time.time()
    try:
        for snr in levels:
            ...
            for split_seed in split_seeds:
                rec_ds, win_ds = work / f"r_{tag}_{split_seed}", work / f"w_{tag}_{split_seed}"
                ...
                for strategy, dataset in (("recording-level", rec_ds), ("window-level", win_ds)):
                    for train_seed in train_seeds:
                        ...
                        print(
                            f"  [{done:3d}/{total}] snr={tag:>4} {strategy:15s} "
                            f"split={split_seed} train={train_seed}  test={acc:6.2%} "
                            f"train={train_acc:6.2%}  ({time.time() - t1:.0f}s/run, "
...
def main() -> None:
    ...
    print(
        f"{len(levels)} SNR x {len(split_seeds)} split x {len(train_seeds)} train x 2 "
        f"= {len(levels) * len(split_seeds) * len(train_seeds) * 2} runs, {EPOCHS} epochs each",
        flush=True,
    )
    runs = run(levels, split_seeds, train_seeds)

    ARTIFACTS.mkdir(parents=True, exist_ok=True)
    (ARTIFACTS / f"{stem}_runs.json").write_text(
        json.dumps([asdict(r) for r in runs], indent=2), encoding="utf-8"
    )
    table = summarise_real(runs)
    print()
    print(table)
    (ARTIFACTS / f"{stem}_table.md").write_text(table + "\n", encoding="utf-8")
    print(f"\nwrote {stem}_runs.json and {stem}_table.md")
