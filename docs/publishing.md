...
can never be reused — `0.5.0` would be burned and the fix would have to ship as
`0.5.1`. Everything below exists to keep that from happening.
...
- [ ] `__version__` in `src/iqforge/__init__.py` matches the tag you are about to
      push (`0.5.0` → `v0.5.0`). This is the only place the version is written;
...
gh release create v0.5.0 --title "v0.5.0" --notes-file docs/release-notes/v0.5.0.md
```
